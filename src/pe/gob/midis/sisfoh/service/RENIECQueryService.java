...
// TODO: La consulta masiva debe ser registrada y programada su ejecuci�n.
	...
	public List<String> consultarPersona(String dni, String usuario, String password) 
		...
		else
		{
			...
			if (params.isApplProxySet()) {
				...
				LOG.info("Estableciendo par�metros del PROXY");
			...
			try {
				...
				while ( ++retriesNumber <= params.getApplReniecWSMaxRetries() && "0001/SINV/UNL".contains(errorCode) )
				{
					...
					if ( !"2/3/4/5/6/8".contains(sessionRENIEC.trim()) )
					{
						...
						// Verificamos que no exista error con la operaci�n anterior
						...
						if ( errorCode.equals("0000") )
						{
							...
							lst.add (lstReturn.get(6));							// C�digo
							lst.add (lstReturn.get(7));							// Restricci�n
						}
						else
						{
							// 0000 : Operaci�n Correcta.
							// 0001 : Usuario no existe.
							// 0002 : El Servidor no puede atender el requerimiento.
							// 5    : Excedi� el m�ximo n�mero de consultas asignadas por d�a.
							// 3    : Esta consultando en un d�a y hora no permitido seg�n convenio.
							...
							// NTP  : No tienen permisos de acceso al m�todo consulta.
							// SINV : C�digo de Sesi�n ingresado inv�lido.
							// DNE  : El DNI consultado es inv�lido.
							// DNV  : El DNI del usuario de la empresa es inv�lido. No est� autorizado a consultar.
							// UNL  : Aplicaci�n que consulta a Web Service no se ha autenticado, debe invocar previamente getSession.
					...
					{
						// 2 : Error en la Operaci�n
						// 3 : La Consulta esta fuera del Horario Permitido
						// 4 : Usuario no valido
						// 5 : La Consulta excedi� la cantidad m�xima permitida
						// 6 : Usuario no existe
						// 8 : contrase�a incorrecta.			
				...
				if ( --retriesNumber ==  params.getApplReniecWSMaxRetries() && !errorCode.equals("0000") ) {
					...
					lst.add("4");
					LOG.error(String.format("Se alcanz� el m�ximo de intentos con la RENIEC"));
				}
				
			} catch (Exception e) {
				LOG.error("Error de Conectividad. No se pudo establecer conexi�n con el servicio web de la RENIEC");
