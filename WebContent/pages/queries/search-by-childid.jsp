...
<mdl:model>
	...
	<jsp:attribute name="head">
		...
		<script src="resources/js/reniec-query-service.js">
		</script>
		<script>
			...
			$(document).ready(function() {
				...
				$('#numberIdFatherForSearch').puiinputtext(); 
				$('#familyTiesForSearch').puidropdown({data: getFamilyTies()});
     	 		$('#familyTiesForSearch').puidropdown('selectValue', 2);
                ...
                $('#search_form').submit(function(event) {
                    ...
                    jBlock();
                     ...
                     var numberIdFatherForSearch =  encodeURI($('#numberIdFatherForSearch').val());
                     var numberIdForSearch       =  encodeURI($('#numberIdForSearch').val());
                     var familyTiesForSearch     =  encodeURI($('#familyTiesForSearch :selected').val());

                     if (numberIdFatherForSearch != numberIdForSearch) {

                         if (familyTiesForSearch != '' && familyTiesForSearch != 'undefined') {
                             
	                        $.getJSON('queryByChildId', 
                            		  {numberIdFather: numberIdFatherForSearch, numberId: numberIdForSearch, familyTies: familyTiesForSearch}, 
			                                ...
			                                if (jsonResponse != null) {
			                                    if (jsonResponse.jsonErrorMessage == null) {
		                                           ...
		                                           $('#messages').puigrowl('show', [{
		                                               severity : 'info',
		                                               summary : "<s:text name='page.search.by.childId'/>",
                     	 	...
                     	 	document.getElementById("search_form").reset();
                  	 		...
                  	 		$('#messages').puigrowl('show', [ {
                                 severity : 'info',
                                 summary : "<s:text name='page.search.by.childId'/>",
                                 detail  : "<s:text name='error.select.family.tie'/>"
                             } ]);
                 	 	...
                 	 	document.getElementById("search_form").reset();
              	 		...
              	 		$('#messages').puigrowl('show', [ {
                             severity : 'info',
                             summary : "<s:text name='page.search.by.childId'/>",
                             detail  : "<s:text name='error.numberid.equals'/>"
                         } ]);
                        jUnblock();
                      }
	...
	<jsp:attribute name="body">
        <div id="messages"></div>
        <span id="title"><s:text name='page.search.by.childId'/></span>
        <form id="search_form">  
        	<div style="margin-top: 20px;">
                <table>
                    <tr>
                        <td class="form-text"><s:text name="datos.numero.dni.busca"></s:text></td>
                        ...
                        <td rowspan="2">
                        	<button id="bt_search" type="submit" title="<s:text name='button.search.tip'/>" tabindex="4">
                        		<s:text name="button.search"/>
                        	</button>
                        </td>
                    </tr>
                    <tr>
                        <td class="form-text"><s:text name="datos.dni.apoderado"></s:text></td>
                        <td><input id="numberIdFatherForSearch"
                        			class="input-code" 
                        			type="text" 
                        			maxlength="8" 
                        			required="required"  
                    ...
                    <tr>
                        <td class="form-text"><s:text name="datos.tipo.vinculo"></s:text></td>
                        <td><select id="familyTiesForSearch" 
                        			name="familyTiesForSearch" 
                        			style="width: 90px;"
                        			tabindex="3"  >
                        	</select></td>
                    </tr>
            ...
            <div id="data" style="margin-top: 20px;" >
			    ...
			    <div>
			        <div id="tab1">
						<table>
							<tr>
								<td id='images' style="vertical-align: top; width: 30%;">
